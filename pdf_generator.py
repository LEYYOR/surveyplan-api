...
def draw_survey_plan(info, result):
    ...
    c.drawCentredString(W/2, ty, "AREA : " + str(area_sqm) + " SQ.METRES (" + str(area_ha) + " HECTS / " + str(acres) + " ACRES)")
    ty -= 3*mm
    sketch_top = ty - 1*mm
    sketch_bot = 48*mm
...
def _draw_main_sketch(c, result, info, sx, sy, sw, sh):
    ...
    p = c.beginPath()
    p.moveTo(ax, ay)
    p.lineTo(ax-2*mm, ay-5*mm)
    p.lineTo(ax+2*mm, ay-5*mm)
    p.close()
    c.drawPath(p, fill=1, stroke=0)
    c.setFont("Helvetica-Bold", 8)
    c.drawCentredString(ax, ay+2*mm, "N")
    ...
    for i, row in enumerate(rows):
        ...
        c.setFont("Helvetica", 5)
        c.drawCentredString(0, -3*mm, str(round(row["distance"],3))+"m")
        c.restoreState()
    ...
    for i, pt in enumerate(coords):
        ...
        beacon = beacons[i].strip() if i < len(beacons) else "EL"+str(5540+i)+"HQ"
