...
def draw_survey_plan(info, result):
    ...
    # Draw traverse polygon
    _draw_main_sketch(c, result, info, sketch_x, sketch_bot, sketch_w, sketch_h)

    # === FIRM DETAILS inside sketch - bottom left corner ===
    # sitting on the title block line
    firm_y = sketch_bot + 2*mm
    c.setFont("Helvetica-Bold", 6); c.setFillColor(BLACK)
    c.drawString(sketch_x + 2*mm, firm_y + 34*mm, info.get("surveyor_name","").upper())
    
    # Office address — display exactly as typed, line by line
...
def _draw_main_sketch(c, result, info, sx, sy, sw, sh):
    ...
    # Station dots and beacon labels
